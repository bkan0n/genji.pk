...
export const frameworkTemplate = `settings
{
	...
	heroes
	{
		Общие
		{
			...
			Зарядка суперспособности пассивная: 0%
...
rule ("OverPy | Player Translation Setup") {
    ...
    actions {
        ...
        Start Facing(Event Player, Direction From Angles(Multiply(10, Index Of Array Value(Global.__overpyTranslationHelper__, String Split(Color(Белый), Empty Array))), 5), 999999999999, To World, Direction and Turn Rate);
    ...
    actions {
        ...
        If(Compare((Event Player).cache_bounceTouched, >=, Null));
            ...
            If(Value In Array(Global.TQ5, (Event Player).cache_bounceTouched));
                ...
                Small Message(Event Player, Custom String("   {0} {1}", Ability Icon String(Hero(Гэндзи), Button(Ultimate)), Value In Array(String Split(Custom String("Ultimate Is ReadyUltimate Is ReadyUltimate Is Ready"), Global.__overpyTranslationHelper__), (Local Player).__languageIndex__)));
            End;
            If(Value In Array(Global.TQ6, (Event Player).cache_bounceTouched));
                ...
                Small Message(Event Player, Custom String("   {0} {1}", Ability Icon String(Hero(Гэндзи), Button(Ability 1)), Value In Array(String Split(Custom String("Dash Is ReadyDash Is ReadyDash Is Ready"), Global.__overpyTranslationHelper__), (Local Player).__languageIndex__)));
            End;
...
rule ("Mechanic | Genji | SUB Reset Abilities") {
    event {
        Subroutine;
        ResetAbilities;
    }
...
rule ("Command | Toggle Split (Hold Ability 1 + Primary + Secondary)") {
    event {
        Ongoing - Each Player;
        All;
        Все;
...
rule ("Command | Toggle Invisible (Hold Ability 1)") {
    event {
        Ongoing - Each Player;
        All;
        Все;
    ...
    actions {
        ...
        If(Global.CompMode);
            ...
            If(And(Global.CompRestartLimit, (Event Player).checkpoint_notLast));
                Small Message(Event Player, If-Then-Else(Compare(String("Ох"), ==, Custom String("噢")), Custom String("   禁止在此比赛中运行期间重新启动"), Custom String("   Restarts In A Run Is Disabled")));
...
rule ("Command | Practice Restart (Hold Interact)") {
    event {
        Ongoing - Each Player;
        All;
        Все;
...
rule ("Command | Practice Skip (Crouch + Primary-Next | Secondary-Previous)") {
    event {
        Ongoing - Each Player;
        All;
        Все;
    ...
    actions {
        ...
        If(Global.CompMode);
            If(Global.instructiontext);
                Create HUD Text((Local Player).comp_instructionHud, Null, Null, Global.instructiontext, Top, -180, Null, Null, Color(Белый), Visible To, Default Visibility);
            End;
            Create HUD Text((Local Player).comp_instructionHud, Custom String("　　　　　Press {0} To Start　　　　　", Input Binding String(Button(Interact))), Null, Null, Top, -176, Color(Белый), Null, Null, Visible To and String, Default Visibility);
...
rule ("Huds | Leaderboard") {
    ...
    actions {
        ...
        Create HUD Text((Local Player).toggle_guide, Null, Custom String(" \\n{0} {1} {0}", Icon String(Flag), Value In Array(String Split(Custom String("Top 5Top 5Top 5"), Global.__overpyTranslationHelper__), (Local Player).__languageIndex__)), Null, Right, -141, Null, Color(Белый), Null, Visible To and String, Visible Always);
        ...
        If(Value In Array(Global.LeaderBoardFull, True));
            ...
            End;
        End;
        Create HUD Text((Local Player).toggle_leaderboard, Custom String("　　　　 {0} {1} {0} 　　　　\\n　　　　　　　　　　　　　　　　　　{2}", Icon String(Flag), Value In Array(String Split(Custom String("LeaderboardLeaderboardLeaderboard"), Global.__overpyTranslationHelper__), (Local Player).__languageIndex__), Evaluate Once(Global.LeaderBoardRemake)), Null, Null, Top, -165, Color(Белый), Null, Null, Visible To and String, Default Visibility);
        Modify Global Variable(LeaderBoardHuds, Append To Array, Last Text ID);
...
rule ("Addon | Pre-set control Map Portal - Toggle Via Workshop Settings") {
    event {
        Ongoing - Each Player;
        All;
        Все;
...
rule ("Addon | Preset Control Map Portal - Toggle Via Workshop Settings") {
    event {
        Ongoing - Global;
    }
    conditions {
    ...
    actions {
        ...
        Else If(Compare(Custom String("{0}", Current Map), ==, Custom String("{0}", Map(Самоа))));
    ...
    actions {
        ...
        While(Compare(Count Of(Global.C), >, 1));
            ...
            While(Compare(First Of(Global.C), <, 256));
                Создать снаряд(Orb Projectile, Null, Add(Value In Array(Global.C, True), Multiply(30, Vector(Subtract(Modulo(First Of(Global.C), 16), 8), False, Subtract(Round To Integer(Divide(First Of(Global.C), 16), Down), 8)))), Down, To World, Heal, Team 1, 0, 0, 30, Good Explosion, Explosion Sound, 1, 1, 0, 0, 0, 0);
        ...
        While(Count Of(Global.C));
            ...
            Создать снаряд(Orb Projectile, Null, First Of(Global.C), Down, To World, Heal, Team 1, 0, 0, 30, Good Explosion, Explosion Sound, 1, 1, 0, 0, 0, 0);
...
rule ("Addon | Caching Jump & Crouch Inputs For Spectators") {
    event {
        Player Joined Match;
        All;
        Все;
...
disabled rule ("Addon | Tournament Instruction Message ◆ 竞赛模式指引消息 <---- INSERT HERE / 在这输入") {
    event {
        Ongoing - Global;
    }
    actions {
...
disabled rule ("Addon | In-World Text For Certain Checkpoints ◆ 特定关卡显示的地图文本 <---- EDIT ME / 在此处编辑") {
    event {
        Ongoing - Global;
    }
    actions {
...
disabled rule ("Addon | Safe Rings - Map Data") {
    event {
        Ongoing - Global;
    }
    actions {
...
disabled rule ("Addon | Genji | Fake Ledge Dash ◆ 超级跳") {
    event {
        Ongoing - Each Player;
        All;
        Все;
...
disabled rule ("Addon | Genji | Fake Triple Jump ◆ 假三段跳") {
    event {
        Ongoing - Each Player;
        All;
        Гэндзи;
