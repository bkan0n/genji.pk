    ...
    <div class="grid max-h-[88vh] overflow-y-auto lg:grid-cols-2">
      ...
      <aside class="relative overflow-hidden border-b border-white/10 bg-zinc-900/40 px-7 py-10 sm:px-10 lg:border-b-0 lg:border-r">
        ...
        <div class="mt-8 grid gap-3 sm:grid-cols-2">
          <div class="rounded-2xl border border-white/10 bg-black/25 p-4">
            <div class="text-xs font-extrabold text-white/90">{{ __('auth.register.private') }}</div>
            <div class="mt-1 text-xs text-zinc-300">{{ __('auth.register.private_desc') }}</div>
          </div>
          <div class="rounded-2xl border border-white/10 bg-black/25 p-4">
            <div class="text-xs font-extrabold text-white/90">{{ __('auth.register.verified') }}</div>
            <div class="mt-1 text-xs text-zinc-300">{{ __('auth.register.verified_desc') }}</div>
          </div>
        </div>
      </aside>
      ...
      <main class="px-7 py-10 sm:px-10">
        <div class="mx-auto max-w-md">
          <h3 class="text-xl font-extrabold tracking-tight text-white sm:text-2xl">{{ __('auth.register.sign_up_title') }}</h3>
          <p class="mt-1 text-sm text-zinc-300">{{ __('auth.register.sign_up_desc') }}</p>
          ...
          <form method="POST" action="{{ url('/register') }}" class="mt-6 space-y-4">
            ...
            <div>
              <label class="block text-xs font-extrabold text-zinc-200">{{ __('auth.register.username_label') }}</label>
              <input
                name="username"
                type="text"
            ...
            <div>
              <label class="block text-xs font-extrabold text-zinc-200">{{ __('auth.register.email_label') }}</label>
              <input
                name="email"
                type="email"
                autocomplete="email"
            ...
            <div>
              <label class="block text-xs font-extrabold text-zinc-200">{{ __('auth.register.password_label') }}</label>
              <input
                name="password"
                type="password"
            ...
            <div>
              <label class="block text-xs font-extrabold text-zinc-200">{{ __('auth.register.password_confirm_label') }}</label>
              <input
