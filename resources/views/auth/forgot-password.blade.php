<div id="forgotPasswordModal" class="fixed inset-0 z-[120] hidden flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true" aria-hidden="true">
  {{-- Backdrop --}}
  <button
...
(() => {
  ...
  backdrop?.addEventListener('click', window.closeForgotPasswordModal);
  closeBtn?.addEventListener('click', window.closeForgotPasswordModal);
  ...
  document.addEventListener('click', (e) => {
    const btn = e.target?.closest?.('[data-open-forgot-password]');
    if (!btn) return;
    e.preventDefault();
    window.openForgotPasswordModal();
  });
