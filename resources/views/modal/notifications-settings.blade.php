  ...
  <div class="relative z-10 flex min-h-full">
    <div class="mx-auto w-full max-w-4xl px-4 pt-24">
      ...
      >
        {{-- ====== Carte Settings ====== --}}
        <article
          class="relative w-full max-w-none overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/80 shadow-2xl backdrop-blur"
        >
          ...
          <header class="flex items-center justify-center px-5 pt-5 pb-3">
            ...
            <button
              id="settingsClose"
              type="button"
              class="absolute right-3 top-3 inline-flex shrink-0 cursor-pointer items-center justify-center rounded-lg border border-white/10 bg-white/5 p-2 text-white/70 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"
              aria-label="{{ __('modals.settings.close') }}"
            >
          ...
          <div class="px-4 pb-5 sm:px-5 overflow-y-auto max-h-[calc(85vh-72px)]">
              ...
              <div class="space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4">
                <div class="flex items-center gap-2">
                  ...
                  <button
                    ...
                    class="inline-flex shrink-0 cursor-pointer items-center justify-center rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500"
                    type="button"
                  >
            ...
            >
              ...
              <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                @if($isEmailAuth)
                  <div class="mb-3 rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-zinc-200">
                    {{ $tr('modals.settings.notifications.discord_unavailable', 'Discord notifications are not available for email accounts.') }}
                  </div>
                @endif

                {{-- Header (desktop) --}}
                <div class="hidden sm:grid grid-cols-4 items-center gap-4 px-3 pb-2 text-[11px] font-extrabold uppercase tracking-wider text-zinc-400">
                ...
                >
                  @foreach($events as $eventType => $label)
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-4 items-start gap-4 rounded-lg border border-white/10 bg-zinc-900/50 px-3 py-2">
                      <div class="min-w-0 text-sm leading-snug text-zinc-200">
                        {{ $label }}
                      </div>
                      ...
                      @foreach($channels as $channel => $channelLabel)
                        ...
                        <div class="flex items-center gap-2 sm:justify-center {{ $disabled ? 'opacity-60' : '' }}">
                          <span class="sm:hidden text-[11px] font-extrabold uppercase tracking-wider text-zinc-400">
                            {{ $channelLabel }}
                          </span>
